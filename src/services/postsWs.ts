...
export function parsePostsLine(line: string): PostsParsedFrame | null {
	...
	if (!trimmed.startsWith('|posts|')) return null;
	const parts = trimmed.split('|');
	if (parts.length < 4 || parts[1] !== 'posts') return null;

	if (parts[2] === 'error') {
		if (parts.length < 5) return null;
		const requestId = parts[3];
		const message = parts.slice(4).join('|');
		return { kind: 'error', requestId, message };
	}
	...
	if (parts.length >= 5) {
		...
		try {
			const json = JSON.parse(jsonStr) as unknown;
			return { kind: 'success', command, requestId, json };
		} catch {
			return null;
		}
	}
...
export class PostsWsClient {
	...
	private appendTextChunk(chunk: string) {
		...
		// Some servers send one complete frame per WS message without a trailing newline.
		// If the buffer itself is a complete frame, parse it now.
	...
	connect(url?: string): Promise<void> {
		...
		return new Promise((resolve, reject) => {
			...
			this.ws.onmessage = ev => {
				...
				// Some servers/browsers deliver WS text frames as Blob/ArrayBuffer.
				// Decode them so command replies resolve instead of timing out.
	...
	/**
	 * Sends `> posts <requestId>` then the command line (e.g. `/list feed 30`).
	 */
