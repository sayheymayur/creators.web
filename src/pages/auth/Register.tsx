						...
						{[
							...
							<button
								...
								className={`w-full text-left p-4 rounded-2xl border-2 transition-all ${
									role === type ? 'border-rose-500 bg-rose-500/10' : 'border-white/10 bg-white/5 hover:border-white/20'
								}`}
							>
								<div className="flex items-start gap-3">
									<div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${role === type ? 'bg-rose-500/20' : 'bg-white/10'}`}>
										<Icon className={`w-5 h-5 ${role === type ? 'text-rose-400' : 'text-white/50'}`} />
									</div>
									<div>
										<p className={`font-semibold text-sm mb-0.5 ${role === type ? 'text-white' : 'text-white/70'}`}>{title}</p>
										<p className="text-xs text-white/40 mb-2">{desc}</p>
										<div className="space-y-0.5">
											{features.map(f => (
												<p key={f} className="text-xs text-white/30 flex items-center gap-1.5">
