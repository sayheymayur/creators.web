...
/** Top bar when a booked timed chat is active and the user is not already inside that thread. */
export function ActiveChatBanner() {
	...
	const active = state.active?.accepted?.kind === 'chat' ? state.active : null;
	const roomId = active?.accepted.room_id ?? '';
	const [bookedRemainingSec, setBookedRemainingSec] = useState<number | null>(null);
	const [ending, setEnding] = useState(false);

	const onThread = roomId && location.pathname === `/messages/${roomId}`;
	...
	const show = useMemo(() => {
		...
		return true;
	}, [active?.accepted?.room_id, state.endedRooms, onThread]);
	...
	const displayName = useMemo(() => {
		if (!active?.accepted?.room_id) return 'Chat';
	...
	return (
		<div className="fixed top-14 left-0 right-0 z-[229] px-3 sm:px-4">
			<div className="max-w-2xl mx-auto bg-surface border border-border/20 rounded-2xl shadow-xl px-4 py-3 flex items-center gap-3">
				...
				<div className="shrink-0 flex items-center gap-2 ml-auto">
					...
					<button
						type="button"
						disabled={ending || !roomId}
						onClick={() => { void navigate(`/messages/${roomId}`); }}
						className="px-3 py-2 rounded-xl text-xs font-bold bg-sky-500 hover:bg-sky-600 text-white transition-colors disabled:opacity-50"
					>
						Continue
					</button>
				</div>
